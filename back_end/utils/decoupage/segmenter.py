...
LOW_PRIORITY_TOKENS = {'and', 'but', 'so', 'then', 'because', 'however', 'also', 'yet', 'or'}
...
def split_chunk_to_phrases(
    ...
    for i, w in enumerate(chunk):
        ...
        if not should_close and (char_len >= max_chars or n_words >= max_words or duration >= max_duration):
            ...
            # last low-priority token
            for j in range(len(cur) - 1, -1, -1):
                if cur[j]["text"].lower().strip() in LOW_PRIORITY_TOKENS and (j + 1) >= min_words:
                    seg = cur[: j + 1]
                    phrases.append(_make_phrase(seg))
                    cur = cur[j + 1 :]
                    found = True
                    reason = "split_at_token_in_long"
                    break
            if found:
                continue
